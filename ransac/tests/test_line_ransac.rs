...
fn params() -> ransac::RansacSettings<Line2d> {
    ransac::RansacSettings {
        max_inlier_error: 0.2,
        max_iters: 100,
...
#[test]
fn still_simple() {
    let line = ransac::run_ransac(
        &vec![Point2d { x: 1., y: 2. }, Point2d { x: 3., y: 4. }],
        &params(),
    );
...
#[test]
fn vertical() {
    let line: Option<Line2d> = ransac::run_ransac(
        &vec![Point2d { x: 0., y: 0. }, Point2d { x: 0., y: 1. }],
        &params(),
    );
    assert_eq!(line, None);
}
